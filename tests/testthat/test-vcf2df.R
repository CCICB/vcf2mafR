test_that("vcf2df works", {
  ...
  expect_error(vcf2df(vcf = path_vcf_unannotated), "Are you sure the VCF is VEP-annotated")
  ...
  expect_error(vcf2df(path_vcf_annotated_no_canonical), regexp = "Failed to find column [CANONICAL] in vep annotations. Please rerun VEP with `Identify Canonical Transcripts` option turned on", fixed = TRUE)

  # Throw error if transcript CANONICAL status not annotated by VEP
  path_vcf_vepped <- system.file("testfiles/test_b38.vepgui.vcf", package = "vcf2mafR")
  df <- expect_error(vcf2maf(path_vcf_vepped, ref_genome = 'hg38'), regexp = NA)
